"""mainsite URL Configuration

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/3.0/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.urls import path, include
from rest_framework.authtoken import views
from drf_jwt_2fa.views import obtain_auth_token, obtain_code_token
from rest_framework_jwt.views import verify_jwt_token, obtain_jwt_token

urlpatterns = [
    path('admin/', admin.site.urls),
    path('auth/', include('djoser.urls')),
    path('auth/admin/', obtain_jwt_token),
    path('auth/token/login', views.obtain_auth_token),
    path('auth/2fa-auth/get-code-token/', obtain_code_token),
    path('auth/2fa-auth/get-auth-token/', obtain_auth_token),
    path('auth/2fa-auth/api-token-verify/', verify_jwt_token),
    path('', include('api.urls')),
    path('staff/', include('staff.urls')),
]
